...
te.pytesseract.tesseract_cmd = 'C:\\Users\\hpattepu\\AppData\\Local\\Tesseract-OCR\\tesseract.exe'
...
def text_extraction(x, img, label=None):
    crop_img = img[int(x[1]):int(x[3]), int(x[0]):int(x[2])]
    txt = te.image_to_string(crop_img, lang='eng+deu',config = '--psm 6')
    txt = txt.replace('\n', ' ').replace('  ', ' ')
    return txt
